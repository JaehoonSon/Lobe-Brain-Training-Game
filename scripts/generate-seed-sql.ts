...
if (!MIGRATION_FILE) {
  console.error(
    "Usage: npx tsx scripts/generate-seed-sql.ts <migration_file_path> [filter_pattern]",
  );
  process.exit(1);
}
    ...
    const gameValues = games
      .map((g: any) => {
        ...
        const instructions = g.instructions
          ? `'${g.instructions.replace(/'/g, "''")}'`
          : "NULL";
        const iconUrl = g.icon_url
          ? `'${g.icon_url.replace(/'/g, "''")}'`
          : "NULL";
        const bannerUrl = g.banner_url
          ? `'${g.banner_url.replace(/'/g, "''")}'`
          : "NULL";
