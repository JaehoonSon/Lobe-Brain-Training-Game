...
export const StoreReview = {
  ...
  getCooldownPeriod: (promptCount: number): number => {
    const index = Math.min(Math.max(promptCount - 1, 0), COOLDOWN_PERIODS.length - 1);
  ...
  requestReview: async (forceDisplay: boolean = false) => {
    try {
      ...
      const lastPrompt = await AsyncStorage.getItem(STORE_REVIEW_LAST_PROMPT_KEY);
      if (lastPrompt) {
      ...
      let promptCount = parseInt(await AsyncStorage.getItem(STORE_REVIEW_PROMPT_COUNT_KEY) || '0', 10);
      ...
      if (!forceDisplay) {
        ...
        const lastRequest = await AsyncStorage.getItem(STORE_REVIEW_LAST_REQUEST_KEY);
        if (lastRequest) {
          ...
          if (Date.now() - lastTime < cooldown) {
            console.log(`StoreReview: Skipped - cooldown active (${cooldown / (24 * 60 * 60 * 1000)} days)`);
            return;
          }
        }
      }
      ...
      await AsyncStorage.setItem(STORE_REVIEW_LAST_PROMPT_KEY, Date.now().toString());
        ...
        [
          {
            ...
            style: 'destructive',
            onPress: async () => {
              await AsyncStorage.setItem(STORE_REVIEW_PROMPT_COUNT_KEY, (promptCount + 1).toString());
              await AsyncStorage.setItem(STORE_REVIEW_LAST_REQUEST_KEY, Date.now().toString());
