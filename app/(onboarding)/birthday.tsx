...
export default function BirthdaySelectionScreen() {
    ...
    const [focused, setFocused] = useState<"month" | "day" | "year" | null>("month");
    ...
    const getInputStyle = (field: "month" | "day" | "year") => {
        ...
        const borderColor = isFocused ? "border-primary" : hasValue ? "border-primary/50" : "border-slate-200";
        return `text-center text-4xl font-bold text-primary p-4 border-b-4 ${borderColor} ${field === 'year' ? 'w-32' : 'w-24'}`;
    };

    return (
        <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
            <View className="flex-1 bg-background px-6" style={{ paddingTop: insets.top, paddingBottom: insets.bottom + 16 }}>
                <View className="flex-1 justify-center gap-8">
                    ...
                    <Animated.View entering={FadeInDown.delay(200).duration(600)} className="flex-row justify-center gap-4">
                        {/* Month */}
                        <View className="items-center gap-2">
                            ...
                            />
                            <Text className={`text-xs font-bold uppercase tracking-wider ${focused === 'month' ? 'text-primary' : 'text-muted-foreground'}`}>Month</Text>
                        </View>
                        ...
                        <View className="items-center gap-2">
                            <TextInput
                                ...
                                onBlur={() => setFocused(null)}
                            />
                            <Text className={`text-xs font-bold uppercase tracking-wider ${focused === 'day' ? 'text-primary' : 'text-muted-foreground'}`}>Day</Text>
                        </View>
                        ...
                        <View className="items-center gap-2">
                            <TextInput
                                ...
                                onBlur={() => setFocused(null)}
                            />
                            <Text className={`text-xs font-bold uppercase tracking-wider ${focused === 'year' ? 'text-primary' : 'text-muted-foreground'}`}>Year</Text>
                        </View>
                    </Animated.View>
                </View>
