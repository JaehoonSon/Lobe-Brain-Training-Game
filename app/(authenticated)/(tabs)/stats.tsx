        ...
        >
            <View className="flex-1">
                ...
                {hasScore ? (
                    <View className="flex-row items-center gap-2">
                        ...
                        <View className="flex-1 h-3 bg-muted rounded-full overflow-hidden">
                            <View
                                ...
                                style={{ width: `${category.progress}%`, backgroundColor: color }}
                            />
                        </View>
                    ...
                    <View className="flex-row items-center gap-2">
                        {/* Empty indicator bar */}
                        <View className="w-2 h-6 rounded-full" style={{ backgroundColor: color }} />
...
export default function StatsScreen() {
    const { overallBPI, categoryStats, totalGamesPlayed, isLoading, error, refresh } =
        useUserStats();

    const hasOverallBPI = overallBPI !== null;
    const categoriesWithData = categoryStats.filter((c) => c.score !== null).length;
                        ...
                        <CardContent className="p-0">
                            ...
                            <View className="p-4">
                                <View className="flex-row items-center justify-between mb-2">
                                    ...
                                    {hasOverallBPI && (
                                        <Text className="text-3xl font-bold text-primary">{overallBPI}</Text>
                                    )}
                                </View>
                                {!hasOverallBPI && (
                                    <P className="text-muted-foreground text-base leading-6">
                                        Your Overall BPI will be available after playing a game from at
                                        least 3 Training Areas below.{" "}
                                ...
                                {hasOverallBPI && (
                                    <P className="text-muted-foreground text-base">
                                        Based on {totalGamesPlayed} games across {categoriesWithData}{" "}
                                        training areas.
                                    </P>
                                )}
                            </View>
