...
CREATE POLICY "Allow read ability norms"
    ON public.game_ability_norms
    FOR SELECT
    TO authenticated
    USING (true);
...
CREATE POLICY "Users can read their own game ability scores"
    ON public.user_game_ability_scores
    FOR SELECT
    TO authenticated
...
CREATE POLICY "Users can read their own ability score history"
    ON public.user_game_ability_history
    FOR SELECT
    TO authenticated
    USING (auth.uid() = user_id);

ALTER TABLE public.user_game_ability_history
    ADD CONSTRAINT uq_user_game_ability_history_daily
...
CREATE POLICY "Users can read their own category ability scores"
    ON public.user_category_ability_scores
    FOR SELECT
    TO authenticated
    USING (auth.uid() = user_id);
...
CREATE POLICY "Users can read their own global ability score"
    ON public.user_global_ability_scores
    FOR SELECT
    TO authenticated
    USING (auth.uid() = user_id);
...
SELECT cron.schedule(
    ...
    '15 0 * * *',
    $$ SELECT public.refresh_ability_scores() $$
);
