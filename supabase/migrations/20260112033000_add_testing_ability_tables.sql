CREATE TABLE IF NOT EXISTS public.testing_game_ability_norms (
    ...
    mean_raw FLOAT8 NOT NULL,
    std_raw FLOAT8 NOT NULL,
...
CREATE POLICY "Allow read testing ability norms"
    ON public.testing_game_ability_norms
    FOR SELECT
    TO authenticated
    USING (true);
...
CREATE POLICY "Users can read their own testing game ability scores"
    ON public.testing_user_game_ability_scores
    FOR SELECT
    TO authenticated
...
CREATE POLICY "Users can read their own testing ability score history"
    ON public.testing_user_game_ability_history
    FOR SELECT
    TO authenticated
    USING (auth.uid() = user_id);

ALTER TABLE public.testing_user_game_ability_history
    ADD CONSTRAINT uq_testing_user_game_ability_history_daily
...
CREATE POLICY "Users can read their own testing category ability scores"
    ON public.testing_user_category_ability_scores
    FOR SELECT
    TO authenticated
    USING (auth.uid() = user_id);
...
CREATE POLICY "Users can read their own testing category ability history"
    ON public.testing_user_category_ability_history
    FOR SELECT
    TO authenticated
    USING (auth.uid() = user_id);

ALTER TABLE public.testing_user_category_ability_history
    ADD CONSTRAINT uq_testing_user_category_ability_history_daily
...
CREATE POLICY "Users can read their own testing global ability score"
    ON public.testing_user_global_ability_scores
    FOR SELECT
    TO authenticated
    USING (auth.uid() = user_id);
...
CREATE POLICY "Users can read their own testing global ability history"
    ON public.testing_user_global_ability_history
    FOR SELECT
    TO authenticated
    USING (auth.uid() = user_id);

ALTER TABLE public.testing_user_global_ability_history
    ADD CONSTRAINT uq_testing_user_global_ability_history_daily
    UNIQUE (user_id, snapshot_date);

CREATE OR REPLACE FUNCTION public.testing_refresh_ability_scores()
RETURNS VOID AS $$
DECLARE
