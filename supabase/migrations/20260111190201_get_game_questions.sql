-- Function to get game questions based on user difficulty
...
BEGIN
  ...
  -- If no record was found, v_difficulty_rating might still be null if we did a direct select
  -- But the COALESCE above only works if a row matches. If NO row matches, we need to handle it.
  ...
  -- 6. Execute Query with ordered pools
  RETURN QUERY
  WITH core_questions AS (
    SELECT q.id, q.content, q.difficulty, 'core'::TEXT as pool_type
    FROM questions q
  ...
  stretch_questions AS (
    SELECT q.id, q.content, q.difficulty, 'stretch'::TEXT as pool_type
    FROM questions q
    WHERE q.game_id = p_game_id
